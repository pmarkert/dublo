  ...
  return page.evaluate(
    ({ config, turnToken: activeTurnToken }) => {
      ...
      const maxControls = Number.isFinite(cfg.maxControls)
        ? Math.max(1, Number(cfg.maxControls))
        : 80;
      const headingSelector =
        typeof cfg.headingSelector === "string" && cfg.headingSelector.trim().length > 0
          ? cfg.headingSelector
          : "h1, h2, h3";
      const maxHeadings = Number.isFinite(cfg.maxHeadings)
        ? Math.max(0, Number(cfg.maxHeadings))
        : 10;
      const alertSelector =
      ...
      const ignoreControlTextPatterns = Array.isArray(cfg.ignoreControlTextPatterns)
        ? cfg.ignoreControlTextPatterns.filter(
            (item) => typeof item === "string" && item.trim().length > 0
          )
        : [];
      ...
      const resolveControlName = (el, textSegments) => {
        ...
        const associatedLabel = id
          ? normalizeText(
              globalThis.document.querySelector(`label[for='${globalThis.CSS.escape(id)}']`)
                ?.innerText || ""
            )
          : "";
        return (
          associatedLabel || textSegments[0] || normalizeText(el.innerText || el.textContent || "")
        );
      };
      ...
      const findActiveModal = () => {
        ...
        for (const el of candidates) {
          ...
          const alertDialogPriority =
            el.getAttribute("role") === "alertdialog" ? 1_000_000_000_000 : 0;
          const score =
            alertDialogPriority + (Number.isFinite(zIndex) ? zIndex : 0) * 1_000_000 + area;
          if (score > bestScore) {
            best = el;
            bestScore = score;
          }
      ...
      const overlayControlSelector =
        "[role='option'], [role='menuitem'], [role='menuitemcheckbox'], [role='menuitemradio'], [role='treeitem']";
      ...
      const scrollContainerIds = new Map();
