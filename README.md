...
Batching is **uncapped by default** — a turn may fill an entire form in one model call. Two natural limits still apply: the observation only lists up to `maxControls` visible controls (default 80), and the model's output-token budget bounds how many actions fit in one response, so very large lists (hundreds of rows) are still done in per-viewport chunks. A hard ceiling of 50 actions per turn guards against a degenerate response. `maxActionsPerTurn` lets you lower that — set it to `1` to disable batching entirely (one action per turn), or to a small number to rein in a model that over-batches. Configure it via `dublo config set max-actions-per-turn <n>`, the workspace default `maxActionsPerTurn`, or `DUBLO_MAX_ACTIONS_PER_TURN`. Batching is the main lever for cutting planner calls on form- and list-heavy flows, and compounds with prompt caching (which cuts the cost of each call).
